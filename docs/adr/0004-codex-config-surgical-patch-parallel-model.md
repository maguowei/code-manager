...
2. **外科补丁所有权(B)**:Codex Apply 只改写 `config.toml` 的 `model_provider` 与对应 `[model_providers.NAME]`,并写 `auth.json` 的 `OPENAI_API_KEY`、把 `auth_mode` 置为 `api_key`(强制 ApiKey 认证,避免既有 ChatGPT OAuth token 顶掉 key);`config.toml` 其余键、注释、顺序**一律原样保留**(需 TOML 保真 round-trip,如 `toml_edit`)。provider 段不写 `env_key`(Codex 只从环境变量读它、不读 auth.json),`wire_api` 恒写 `responses`(Codex 已移除 chat 协议)。Codex Profile 因此是「一层 provider+key 覆盖」,**不是**「一份完整设置单元」。
