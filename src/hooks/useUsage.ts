...
export function useUsage(): UseUsageResult {
  ...
  const reload = useCallback(() => reloadWith(filterRef.current), [reloadWith]);

  // filter 变化 → 重新查询
  useEffect(() => {
    void reloadWith(filter);
  }, [filter, reloadWith]);
  ...
  const scheduleReload = useCallback(() => {
    ...
    debounceRef.current = setTimeout(() => {
      void reloadWith(filterRef.current);
    }, POLL_DEBOUNCE_MS);
  }, [reloadWith]);
  ...
  const refreshPricing = useCallback(async () => {
    ...
    try {
      ...
      await reloadWith(filterRef.current);
    } catch (e) {
      setError(typeof e === "string" ? e : String(e));
      throw e;
    } finally {
