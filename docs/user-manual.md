...
多数页面的新增或编辑操作都会打开右侧抽屉。保存前，抽屉内通常会提供控件模式、JSON 模式或预览，方便先检查最终配置。
...
- 成本按当前价格表估算。
- 未识别模型的 Token 会统计，但成本按 0 计。
- 价格表加载顺序是本地缓存、内置兜底、启动后异步从 models.dev 更新。
...
4. 切到按会话表格，点击异常会话查看消息级明细。
5. 如果模型显示在未识别模型列表中，说明成本按 0 计，需要更新价格表或等待价格表支持。
...
如果模型不在价格表中，Token 会统计，但费用按 0 计。可以点击刷新价格尝试更新价格表。
