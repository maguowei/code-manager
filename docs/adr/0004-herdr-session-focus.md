...
- Ghostty 没有 pid/tty AppleScript API；命名会话的 title 可区分同 cwd 的多个 client，title 未命中且 cwd 多匹配时宁可降级为内部聚焦成功，也不误激活其它会话。
