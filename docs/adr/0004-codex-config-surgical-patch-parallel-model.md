...
4. **Provider 允许自定义(B)**:Codex Provider 可由用户新建(填 `base_url`/`env_key` 名;`wire_api` 固定 `responses`,不暴露),内置只读项仅作快速起步预设。这打破了「Provider 全部内置只读」这条**只对 Claude 成立**的不变量,因为切私有中转正是 Codex 切换的主力场景。
