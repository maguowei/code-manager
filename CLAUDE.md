...
常用工作流优先看 `Makefile`：`make init` 安装依赖，`make dev` 等价 `pnpm tauri dev`，`make build` 等价 `pnpm tauri build`，`make test` 顺序运行 Rust 与前端测试，`make lint` 跑 Rust clippy（含 test target），`make check` 只跑 Rust `cargo check`。
