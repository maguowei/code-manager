...
| Profile / Preset | `src/components/ProfileEditor.tsx`、`src/components/PresetEditor.tsx`、`src/components/profile-editor/`、`src-tauri/src/config.rs` | 合并和落盘权威逻辑在 Rust；前端不要复制配置合并逻辑。 |
...
| Token 用量 | `src/components/UsagePage.tsx`、`src-tauri/src/usage.rs` | 数据源是 `~/.claude/projects/**/*.jsonl`，SQLite 只做缓存和索引。 |
| 项目与诊断 | `src/components/ProjectsPage.tsx`、`src-tauri/src/project.rs`、`src-tauri/src/logging.rs` | 日志不能记录密钥、Token、完整 settings、Memory 内容或 Skill 文件内容。 |
...
完成后运行 git diff --check，并检查是否有乱码、过期版本号或断链。
```
