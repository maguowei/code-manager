...
| 项目列表与分支 / Worktree 清理 | ✅ | ✅ | ✅ | `src-tauri/src/project.rs` |
...
| SQLite (`usage.db`) | `~/Library/Application Support/com.gotobeta.app.ai-manager/` | `$XDG_CONFIG_HOME/com.gotobeta.app.ai-manager/` | `%APPDATA%\com.gotobeta.app.ai-manager\` | `tauri-plugin-sql` 默认 `app_config_dir()` |
...
**注意：只有应用数据走这个非标准路径。SQLite 与日志走 Tauri 插件默认路径**（`tauri-plugin-sql` 用 `app_config_dir()`，`tauri-plugin-log` 用 `app_log_dir()`），在 macOS 上仍然是 `~/Library/...`。不要把这条规则误推到 SQLite 或日志上。
