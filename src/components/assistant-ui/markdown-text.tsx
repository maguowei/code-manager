...
// 仅注解为基础 Components（HTML 元素覆盖映射）以获得回调参数类型推断；
// CodeHeader 是 StreamdownTextComponents 的专用槽位，其 props 与 Components 字符串索引签名不兼容，
// 在传入 components 时再并入。
...
// CodeHeader 槽位的 props 与基础 Components 的字符串索引签名在类型层面互斥
// （streamdown 自身类型缺陷），此处显式合并并 cast 一次，运行时形状正确。
const components = {
  ...elementComponents,
  CodeHeader,
