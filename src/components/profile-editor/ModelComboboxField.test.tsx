...
const SUGGESTIONS = ["deepseek-flash[1m]", "deepseek-v4.1-flash[1m]"];
...
describe("ModelComboboxField", () => {
  ...
  it("点击触发按钮展开候选并可选中回填", () => {
    ...
    const option = screen.getByText("deepseek-v4.1-flash[1m]");
    fireEvent.click(option);
    expect(onChange).toHaveBeenCalledWith("deepseek-v4.1-flash[1m]");
  });

  it("按当前输入值过滤候选", () => {
    renderField({ value: "v4.1" });
    fireEvent.click(screen.getByRole("button", { name: "选择模型" }));
    expect(screen.getByText("deepseek-v4.1-flash[1m]")).toBeInTheDocument();
    expect(screen.queryByText("deepseek-flash[1m]")).not.toBeInTheDocument();
  });
});
