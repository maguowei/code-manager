...
/**
 * 对话详情应用内查找。Cmd/Ctrl+F 打开查找栏；在 containerRef 内精确高亮全部匹配词，
 * ↑/↓ 循环跳转并滚动到当前匹配。依赖 CSS Custom Highlight API，不支持时降级为仅滚动定位。
 * containerRef 应指向「仅含消息」的元素（不含查找栏自身），避免计数把查找栏文本算进去。
 */
export function useConversationSearch(containerRef: RefObject<HTMLElement | null>) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [matchCount, setMatchCount] = useState(0);
  const [currentIndex, setCurrentIndex] = useState(0);
  const supported = useMemo(() => !!getHighlightRegistry() && !!getHighlightCtor(), []);
  ...
  // 拦截 Cmd/Ctrl+F：打开查找栏并聚焦输入框（替代 WebView 无效的默认查找）
  useEffect(() => {
  ...
  // 捕获阶段处理 Esc：查找栏开启时只关查找栏，阻止 Radix Sheet 连带关闭整个抽屉
  useEffect(() => {
    if (!open) return;
  ...
  // query / open 变化后（防抖）重算匹配并设置高亮；effect 在 DOM 更新后运行，侧链已展开
  useEffect(() => {
    if (!open) return;
  ...
  }, [open, query, containerRef, clearHighlights, applyCurrent]);

  // 卸载时清理高亮，避免残留在全局 CSS.highlights 注册表
