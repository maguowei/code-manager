...
const stopReminderPath = join(
	repoRoot,
	".claude/hooks/stop-quality-reminder.mjs",
);
...
describe("pre-push verify hook", () => {
	it("skips local verify for tag-only pushes", () => {
		const result = runPrePushVerify(
			"refs/heads/codex-hook abc refs/tags/v0.20.0 def\n",
		);
	...
	it.skipIf(platform() === "win32")(
		...
		() => {
			...
			const result = runPrePushVerify(
				"refs/heads/codex-hook abc refs/heads/codex-hook def\n",
				{
			...
			expect(result.stdout).toContain(
				"[1/4] Rust 格式检查: make fmt-rust-check",
			);
	...
	it.skipIf(platform() === "win32")(
		"stops at the first failed verification step",
		() => {
...
function createGitRepo() {
	...
	execFileSync(
		"git",
		["config", "user.email", "agent-guardrails@example.com"],
		{ cwd },
	);
...
function processEnvPath() {
	return execFileSync("node", ["-p", "process.env.PATH ?? ''"], {
		encoding: "utf8",
	}).trim();
}
