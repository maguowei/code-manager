...
| 前端完整 | `pnpm biome:ci`、`pnpm build`、`pnpm test` |
...
| 全量本地门禁 | `make lint`、`make test` |

本地启动桌面应用用 `pnpm tauri dev` 或 `make dev`；`pnpm dev` 只启动 Vite。生产包用 `pnpm tauri build` 或 `make build`。`make fmt` 与 `pnpm check` 会改写文件，避免把它们当只读验证。
