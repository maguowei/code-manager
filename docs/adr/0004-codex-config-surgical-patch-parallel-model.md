...
2. **外科补丁所有权(B)**:Codex Apply 只改写 `config.toml` 的 `model_provider` 与对应 `[model_providers.NAME]`,并写 `auth.json` 的 `openai_api_key`;`config.toml` 其余键、注释、顺序**一律原样保留**(需 TOML 保真 round-trip,如 `toml_edit`)。Codex Profile 因此是「一层 provider+key 覆盖」,**不是**「一份完整设置单元」。
